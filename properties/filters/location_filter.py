...
class LocationFilter(django_filters.FilterSet):
    ...
    # getting the longitude& latitude and filtering at point( longitude,latitude ) so we pop to skip filteration with longitude or  latitude
    ...
    def filter_queryset(self, queryset):
        ...
        if distance is  None :distance=3000 # default  be 3000 meters
        else :distance =Decimal(distance)

        print(distance)
        filtered_queryset  = filter(lambda p: dist_calc(user_latitude, user_longitude, p.latitude, p.longitude) <= distance, queryset) #get 
